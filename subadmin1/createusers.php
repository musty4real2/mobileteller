...
if($confirmpass!=$password){$error[]="the two passwords do not match each other";}
	...
	if(!empty($error)){
		...
		elseif(empty($error)){
			...
			$sql=mysql_query("INSERT INTO `users` (`name`, `address`, `phone_number`, `username`, `password`, `dept`, `entry_date`) VALUES ('$name', '$address', '$phone_number', '$username', '$password', '$dept',NOW())");

			if($sql){
			
		header("location:admin_menu.php?msg=User has been succesfully created&id={$_GET['id']}&noble=noble&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief");
			}
			else{
...
<div class="well">
    <ul class="nav nav-tabs">
      <li class="active"><a href="#home" data-toggle="tab">You are about to create a new user</a></li>
      <li></li>
    </ul>
    ...
    <form id="tab" action="<?php $_SERVER['PHP_SELF'];?>" method="post">
        <label>Name</label>
        <input type="text" class="input-xlarge" name="name" value="<?php if($_POST['name']){echo $_POST['name']; } ?>" autocomplete="off">
        <label>Address</label>
       <textarea name="address" value=""><?php if($_POST['address']){echo $_POST['address']; } ?></textarea>
        <label>phone Number</label>
        <input type="text" class="input-xlarge" name="phone_number" autocomplete="off" value="<?php if($_POST['phone_number']){echo $_POST['phone_number']; } ?>">
        <label>Username</label>
        <input type="text"  class="input-xlarge" name="username" autocomplete="off" value="<?php if($_POST['username']){echo $_POST['username']; } ?>">
        <label>Employee Department</label>
        <input type="text" class="input-xlarge"  autocomplete="off" name="dept" value="<?php if($_POST['dept']){echo $_POST['dept']; } ?>">
<label>Password</label>
        <input type="text"  autocomplete="off" class="input-xlarge" name="password" value="<?php if($_POST['password']){echo $_POST['password']; } ?>">
        <label>Confirm Password</label>
        <input type="text" autocomplete="off" class="input-xlarge" name="confirmpass" value="<?php if($_POST['confirmpass']){echo $_POST['confirmpass']; } ?>">
        <br/>
        <br/>
        <input type="submit"  class="btn btn-primary" value="Create User" name="a"/>
    </form>
      </div>
