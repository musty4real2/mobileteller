...
if($cardno==""){
	...
	$fetchcard=@mysql_query("SELECT * FROM `pin` WHERE `pin`='$cardno' AND `status`='1'") or die(mysql_error());
			...
			$s1=@mysql_query("SELECT * FROM `suscriber` WHERE `id`='{$_GET['deposite']}'") or die(mysql_error());;
				...
				$history="INSERT INTO `cardhistory` (`name`, `phone_number`, `pin`, `serial`,`cardamount`, `entry_date`, `time`, `client_id`, `staff_id`) VALUES ('$name', '$phone_number', '$cardpin', '$cardserial','$cardamount', NOW(), NOW(), '{$_GET['deposite']}', '{$_GET['id']}')";

$historyin=mysql_query($history) or die("INSERT ERROR".mysql_error());
	...
	$query=mysql_query($update) or die("UPDATE ERROR:".mysql_error());
				...
				//if($update){
//		$a="http:www.linuxsms.com/components/com_smsreseller/smsapi.php?username='Mobile teller'&password='priceless1'&sender='Mobile Teller'&message='message'&recipient='2348061662025'";
