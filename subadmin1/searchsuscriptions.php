...
<div class="btn-toolbar">
    <button class="btn btn-primary"><i class="icon-save"></i> You are about to search through all created users as suscribers</button>
  <div class="btn-group">
...
?>
    <legend>Search Using Phone Number</legend>
...
$select="SELECT * FROM `suscriber` WHERE `phone_number`='{$_GET['wname']}'";
...
<?php } } }?>
    ...
    <legend>Search Using Surname</legend>
...
<tr>
  <td>
<input style=" padding-left:20px; font-size:20px; font-weight:bold; background-image:url(../images/search_field.jpg); width:582px; height:47px; background-repeat:no-repeat; border:none;" type="text" name="wname2" id="wname2" maxlength="30" autosuggest="off" size="40" value="<?php if(isset($_GET['wname2'])){echo $_GET['wname2'];} ?>" placeholder="Enter surname to search with" /></td>
<td>
<input style="border:none; background-image:url(../images/search_butt.jpg); width:132px; height:49px; background-repeat:no-repeat; cursor:pointer;" type="submit" name="search2" id="button2" value=" " />
</td>
</tr>
</table>
</form></center>
...
if(isset($_GET['search2'])){
	include("connect.php");
$name2=addslashes($_GET['wname2']);
$name2.=addslashes($_GET['id']);

	if(empty($name2)){$error[]="Please enter a surname to search with";}
...
$select="SELECT * FROM `suscriber` WHERE `surname` LIKE'%{$_GET['wname']}%'";
$q=mysql_query($select);
...
<td><?php echo $l['phone_number'];?></td>
   ...
   <td><?php echo $l['marital_status'];?></td>
      <td><?php echo $l['sex'];?></td>
       ...
       <td><?php echo $l['entry_date'];?></td>
      <td>
            <?php echo "<a href='single_subscription.php?confirmid=$rid&id={$_GET['id']}&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief'><i class='icon-user'></i></a>"; ?></td>
        </tr>
	<?php } ?>
</table>
</fieldset>
