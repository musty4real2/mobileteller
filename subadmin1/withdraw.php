...
<input type="text" name="amount" autocomplete="off"/><br/>
<label>Confim amount to withdraw</label>
<input type="text" name="camount" autocomplete="off"/><br/>
<label>Enter Withdrawal Details</label>
<textarea name="narration"></textarea><br/>
<input type="hidden" name="deposite" value="<?php echo $_GET['deposite']; ?>"
<br/>
<input type="submit" name="a" value="Withdraw"/>
...
$camount=mysql_real_escape_string($_POST['camount']);
$narration=mysql_real_escape_string($_POST['narration']);

if($amount==""){
		$error[]="Please enter an amount you want to withdraw";
		}
		if($amount!=$camount){
		$error[]="the two amount entered do not match each another";
		}
		if($narration==""){
		$error[]="Please enter a narration for withdraw";
		}

	if(!empty($error)){
		echo "<h1 class='warning'>ERROR:Please complete the following item</h1><br/>";
	...
	if(empty($error)){
		...
		$pick="SELECT `balance` FROM `suscriber`	WHERE `id`='{$_GET['deposite']}'";
		$p=mysql_query($pick)or die(mysql_error()) ;
		while ($me=mysql_fetch_array($p)){
			$oldqty=$me['balance'];
			}
			
						$newamount=$oldqty-$amount;
						
							
	$balance=$qo-$qd;
	$update="UPDATE `suscriber` SET `balance`='$newamount' WHERE `id`='{$_GET['deposite']}'";
...
VALUES ('{$_GET['deposite']}', '$amount', NOW(),NOW(),'$narration','{$_GET['id']}')";

$addin=mysql_query($add) or die("INSERT ERROR".mysql_error());

					header("location:withdrawconfirmation.php?id={$_GET['id']}&noble=noble&thief=thief&confirmid={$_GET['deposite']}&noble=noble&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief&msg=you have withdrawn the sum of N $amount");
...
?>
      ...
      <p class="pull-right"><a href="#" target="_blank">powered by</a> by <a href="#" target="_blank">mustymeena</a></p>
                        ...
                        <p>&copy; 2014 <a href="http://www.jobconnectng.org" target="_blank">jobconnect nigeria</a></p>                    </footer>
