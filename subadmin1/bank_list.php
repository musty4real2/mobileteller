...
<?php
  ...
  $q = "SELECT * FROM `suscriber` WHERE `bank_name`='{$_GET['bank']}'";
  $r = mysql_query ($q);
...
?>
        <?php
		$get="SELECT * FROM `suscriber` WHERE `bank_name`='{$_GET['bank']}' ORDER BY `id` DESC LIMIT $start, $display ";
		$get=$object->query($get);
		if(mysql_num_rows($get)==0){ echo "<p style='margin:30px 0 30px 0; text-align:center; border:1px solid #CCC;'>No List</p>";}
		
        
if(mysql_num_rows($get)>0){
?>
<center><h2><?php $a=strtoupper($_GET['bank']).'  LIST OF SUCRIBER(S)';
echo $a;
 ?></h2></center>
    <table class="table">
      <thead>
...
if ($current_page != 1) {
 echo '<center><a href="bank_list.php?s=' .
($start - $display) . '&p=' . $pages .'&bank='.$_GET['bank'].'&id='.$_GET['id'].'">Previous</a></center> ';
 }

for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="bank_list.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages .'&bank='.$_GET['bank'].'&id='.$_GET['id'].'">' . $i . '</a> ';
 } else {
 echo $i . ' ';
}
 }
...
if ($current_page != $pages) {
 echo '<center><a href="bank_list.php?s=' .
($start + $display) . '&p=' . $pages .'&bank='.$_GET['bank'].'&id='.$_GET['id'].'&bank='.$_GET['bank'].'&id='.$_GET['id'].'">Next</a></center>';
 }
