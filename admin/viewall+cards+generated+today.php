  ...
  <!--<![endif]-->
    ...
    <div class="navbar">
        ...
        <div class="header">
            ...
            <h1 class="page-title">View All Pin(s) Generated Today  <?php echo $object->currentDate(); ?></h1>
        </div>
            ...
            <li class="active">View Generated Pin(s)  Details</li>
        </ul>
...
<div class="btn-toolbar">
    <button class="btn btn-primary"><i class="icon-save"></i> You are Viewing all pin(s) generated today </button>
  <div class="btn-group">
...
echo 'Do You Really Want To Delete these if you do it would not be available ? <a href="viewall+cards+generated+today.php?yesdelete3='.$_GET['deleteid3'].'&id='.$_GET['id'].'">Yes </a>| <a href="viewall+cards+generated+today.php?id='.$_GET['id'].'">No</a>';
exit();
...
if(isset($_GET['yesdelete3'])){
	...
	$sql=mysql_query("DELETE FROM `pin` WHERE `id`='$id_to_delete' limit 1") or die(mysql_error());
	
	header("location:viewall+cards+generated+today.php?id={$_GET['id']}");
	exit();
}
?>
<?php
  ...
  $q = "SELECT * FROM `pin` WHERE `entry_date`=CURDATE() ORDER BY `id` ";
  $r = mysql_query ($q);
...
?>
        <?php
		$get="SELECT * FROM `pin` WHERE `entry_date`=CURDATE() ORDER BY `id` DESC LIMIT $start, $display ";
		$get=$object->query($get);
		if(mysql_num_rows($get)==0){ echo "<p style='margin:30px 0 30px 0; text-align:center; border:1px solid #CCC;'>No Cards generated today yet</p>";}
...
<td><?php echo $l['serial'];?></td>
   ...
   <td><?php echo $l['status'];?></td>
      ...
      <td>
              ...
              <?php
              // echo "<a href='viewall+cards+generated+today.php?deleteid3=$rid&id={$_GET['id']}&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief'><i class='icon-remove'></i></a>";
			   ?>
          </td>
        </tr>
         <?php 
...
if ($current_page != 1) {
 echo '<center><a href="viewall+cards+generated+today.php?s=' .
($start - $display) . '&p=' . $pages .
'&id='.$_GET['id'].'">Previous</a></center> ';
 }

for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="viewall+cards+generated+today.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '&id='. $_GET['id'].'">' . $i . '</a> ';
 } else {
 echo $i . ' ';
}
 }
