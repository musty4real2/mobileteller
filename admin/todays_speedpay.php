...
?>
                <?php
		$get="SELECT * FROM `speedpay_history` WHERE `entry_date`=CURDATE() ORDER BY `id` DESC LIMIT $start, $display ";
		$get=$object->query($get);
		if(mysql_num_rows($get)==0){ echo "<p style='margin:30px 0 30px 0; text-align:center; border:1px solid #CCC;'>No TRANSACTIONS FOUND</p>";}
		
        
if(mysql_num_rows($get)>0){
?>
  <h2>LIST OF ALL SPEED PAY TRANSACTIONS AS AT <?php echo date("d").'   '; ?><?php echo date("M").'   '; ?><?php echo date("Y");?></h2>
    <table class="table">
      <thead>
          <tr>
        <th>&nbsp;</th>
...
<td><?php echo $l['card_serial'];?></td>
   ...
   <td><?php echo $l['senders_phone'];?></td>
	 ...
	 <a  onclick="return confirm('Do you really want to See these details?')" href="<?php echo "single_speed_h.php?print=$rid&id={$_GET['id']}&noble=noble&thief=thief&deposite={$_GET['confirmid']}&noble=noble&thief=thief&noble=noble&thief=thief&noble=noble&thief=thief";?>">View</a>
