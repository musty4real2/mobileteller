  ...
  <!--<![endif]-->
    ...
    <div class="content">
        ...
        <div class="header">
            ...
            <h1 class="page-title">View All Users (Distributors)</h1>
        </div>
            ...
            <li class="active">View distributor(s)  details</li>
        </ul>
...
<div class="btn-toolbar">
    <button class="btn btn-primary"><i class="icon-save"></i> You are Viewing all created users as distributor(s)</button>
  <div class="btn-group">
...
echo 'Do You Really Want To Delete these if you do it would not be available ? <a href="viewdistributors.php?yesdelete3='.$_GET['deleteid3'].'&id='.$_GET['id'].'">Yes </a>| <a href="viewdistributors.php?id='.$_GET['id'].'">No</a>';
exit();
...
if(isset($_GET['yesdelete3'])){
	...
	$sql=mysql_query("DELETE FROM `distributors` WHERE `id`='$id_to_delete' limit 1") or die(mysql_error());
	
	header("location:viewdistributors.php?id={$_GET['id']}");
	exit();
}
?>
<?php
...
?>
        <?php
		$get="SELECT * FROM `distributors` ORDER BY `id` DESC LIMIT $start, $display ";
...
<td><?php echo $l['phone_number'];?></td>
   <td ><?php echo $l['address'];?></td>
          <td>
                            ...
                            <?php echo "<a href='editdistributors.php?editid=$rid&id={$_GET['id']}'><i class='icon-pencil'></i></a>"; ?>

              <?php echo "<a href='viewdistributors.php?deleteid3=$rid&id={$_GET['id']}'><i class='icon-remove'></i></a>"; ?>
          </td>
        </tr>
         <?php 
}
...
if ($current_page != 1) {
 echo '<center><a href="viewdistributors.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="viewdistributors.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '&id='.$id.'">' . $i . '</a> ';
 } else {
 echo $i . ' ';
