...
<?php
  ...
  $q = "SELECT * FROM `speedpay_history` WHERE `entry_date`='{$_GET['cdate']}'";
  $r = mysql_query ($q);
...
?>
<?php
$select="SELECT * FROM `speedpay_history` WHERE `entry_date`='{$_GET['cdate']}' ORDER BY `id` DESC LIMIT $start, $display ";
...
<legend><?php echo mysql_num_rows($q);?> Result(s)   <br/><b>LIST OF ALL SPEED PAY TRANSACTIONS AS AT <?Php echo  $_GET['cdate']; ?></b>
</legend>
    <table class="table">
      <thead>
        <tr>
