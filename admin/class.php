   ...
   <?php echo $this->imageResize($pic[0], $pic[1], $this->scale);?> />
	...
	while($row=mysql_fetch_array($fetchub)){
		return $row['title'].' '.$row['surname'].' '.$row['first_name'].' '.$row['middle_name'];
			...
			public function totalSpeedMoney1(){
		
		$query1="SELECT SUM(card_amount) FROM `speedpay_history` WHERE `entry_date`=CURDATE()";
		$result1=mysql_query($query1) or die(mysql_error());

		while($row=mysql_fetch_array($result1)){
		$tot=$row['SUM(card_amount)'];
		if($tot==0){$tot="0.00";}
	
		echo "<span style='text-decoration:line-through; font-weight:bold;'>N</span>".number_format($tot);

		}//while
		
		}//generateSummaryByBank
		
					public function totalSpeedMoney11(){
		
		$query1="SELECT SUM(fee) FROM `speedpay_history` WHERE `entry_date`=CURDATE()";
		$result1=mysql_query($query1) or die(mysql_error());
		...
		public function totalSpeedMoney12(){
				$count=$this->query("SELECT COUNT(*) AS `total` FROM `speedpay_history` WHERE `entry_date`=CURDATE()");
				
				while ($row=mysql_fetch_array($count)){
				$g=50 * $row['total'];
				}
				$count1=$this->query("SELECT SUM(fee) FROM `speedpay_history` WHERE `entry_date`=CURDATE()");
