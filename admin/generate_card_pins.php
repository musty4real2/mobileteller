...
<div class="row-fluid">
<legend>Generate Speed Pay Cards</legend>
...
<input type="hidden" name="cardtype" value="speed pay" >
<br/>
<?php
$dd=$object->query("SELECT * FROM `distributors` ORDER BY `name` ASC");
 
 ?>

 <select   class="input" name='distribute'>
...
echo "<option value='$dname ($dphone)'> $dname</option>";
}
 ?>
 </select> 
<br/>
...
$distribute=mysql_real_escape_string($_POST['distribute']);
include("connect.php");
if(isset($_POST['a'])){
...
mysql_query("INSERT INTO pin (`serial`,`pin`,`amount`,`status`,`entry_date`,`card_type`,`distributor`) values ('$serial','$pin','$denominations','1',NOW(),'$cardtype','$distribute')");
$sn = mysql_query("SELECT `serial`,`pin`,`amount`,`entry_date`,`card_type` FROM `pin` WHERE `pin`='$pin' AND `serial`='$serial'");
while ($row1 =mysql_fetch_array($sn)) {
$pinSerial = $row1["serial"];
$amount = $row1["amount"];
$date = $row1["entry_date"];
$cardt = $row1["card_type"];
}
...
(mysql_num_rows($check)==0) {
mysql_query("INSERT INTO pin (`serial`,`pin`,`amount`,`status`,`entry_date`,`card_type`,`distributor`) values ('$serial1','$pin1','$denominations1','1',NOW(),'$cardtype1','$distribute2')");
